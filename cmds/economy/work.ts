export default {
  ...
  run: async ({sock, m, args, command, text, prefix}) => {
    ...
    if (remainingTime > 0) {
      return sock.reply(m.chat,`✿ Debes esperar *${msToTime(remainingTime)}* para trabajar de nuevo.`,m, m.rcanal);
    }
   ...
   await updateChatUser(m.chat, m.sender, 'workCooldown', user.workCooldown)
        ...
        await sock.reply(m.chat, `「✿」 ${pickRandom(trabajo)} *¥${rsl.toLocaleString()} ${monedas}*.`, m, m.rcanal)
  }
};
