...
const canalSeleccionado = canales[Math.floor(Math.random() * canales.length)]
  ...
  const pfp = botSetting.icon || ''
    const canalI = botSetting.id || '120363420992828502@newsletter'
    const canalNam = botSetting.nameid || '𐚁๋࣭⭑ֶָ֢ ѕтєℓℓαя ωα ⚡︎ ¢нαηηєℓ  ₍ᐢ..ᐢ₎♡'
      ...
      jid,
      {
        extendedTextMessage: {
          ...
          contextInfo: {
            ...
            externalAdReply: {
              title: namebugsito,
              body: `${namebugg2}, Built With 💎 By Diamond`,
              mediaType: 1,
              renderLargerThumbnail: false,
              previewType: `PHOTO`,
              thumbnailUrl: pfp,
  ...
  client.sendContextInfoIndex = async (
    ...
    const namebot2 = config.body || `${settings.namebot2}, Built With 💎 By Diamond`
    const canalId = settings.id ||'120363350554513092@newsletter'
    const canalName = settings.nameid || '💎̵̱͞㌔͜𓏲𝐓̸͟һ̴̊ᥱ̤𓍲̈͜𝐃̷і̤ᥲ̽͜m̶͠᥆̷̟ᥒ᪶͡ძ̴᪶𓊓̵̬𝐂̸͔һ͡ᥲ͓ᥒ̴͍ᥒ̷̈ᥱ̤ᥣ̵⋆͙̈么͟͞──'
    const sourceUrl =
