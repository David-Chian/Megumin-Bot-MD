...
if (psmap == '') return conn.reply(m.chat, `😿 No se ha encontrado usuarios para hacer el pvp 1vs1`, m, fake)

    const taguser = '@' + m.sender.split('@s.whatsapp.net')[0];

    // Verificar si se proporcionaron los argumentos necesarios
    if (args.length < 2) {
         await  conn.reply(m.chat, `🚩 Debes proporcionar la hora (HH:MM) y el pais (CO, CL, AG, MX)\n\nEjemplo:\n» ${usedPrefix}1vs1 10:10 CO`, m, rcanal);
        return;
    }
...
mentions: [m.sender]});*/
m.reply(message, null, {mentions: [user0, user1, user2, user3]})   
};
handler.help = ['1vs1'];
handler.tags = ['ff'];
